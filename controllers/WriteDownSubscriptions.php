...
//        }
    ...
    public function answerToUser($message, $answer, $redirect)
    {
        if ($redirect == static::REDIRECT_SUCCESS){
            HandleHook::writeDownCurrentLevel($message->getChat()->id, 1);
    ...
    public function writeDownSubscription($message, $pageID, $social_network_name)
    {
        ...
        $tg_sub = TgSubscription::find()->where(['user_id' => $message->getChat()->id, 'page_id' => $page->page_id])->one($this->db);
        if ($tg_sub == null) {
            $tg_sub = new TgSubscription;
            $tg_sub->user_id = $message->getChat()->id;
